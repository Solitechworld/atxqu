...
<img src="assets/banner.svg" alt="ATXQU — Address Transaction Query Utility" width="100%">

<p>
<img src="https://img.shields.io/badge/python-3.8%2B-00d9d9?style=flat-square&labelColor=07090f" alt="Python 3.8+">
<img src="https://img.shields.io/badge/deps-stdlib%20only-00c850?style=flat-square&labelColor=07090f" alt="stdlib only">
<img src="https://img.shields.io/badge/platform-macOS%20%C2%B7%20any-00d9d9?style=flat-square&labelColor=07090f" alt="macOS / any">
<img src="https://img.shields.io/badge/licence-MIT-8b96a8?style=flat-square&labelColor=07090f" alt="MIT">
</p>

**Address Transaction Query Utility** — a local, dependency-free web app that
finds every **spent** transaction of a Bitcoin address (every transaction where
the address appears in the **inputs**), normalizes each to a fixed JSON schema,
and streams them into one combined file. Companion data-acquisition front-end to
[vusi](https://github.com/Solitechworld/vusi): ATXQU turns an address into spent
transactions; vusi turns those into `(r, s, z, pubkey)` tuples and runs ECDSA
nonce attacks over them.

![ATXQU dashboard](assets/screenshot.png)

*The dashboard, running locally on `127.0.0.1:8787`: paste an address (or upload
a `.txt` list), pick a provider and concurrency, and watch spent transactions
stream in with live telemetry. Stdlib only — no GPU, because the work is
network-bound, not math-bound.*
...
This copy of ATXQU is the data-acquisition front-end for [vusi](https://github.com/Solitechworld/vusi). It works standalone, or as its
...
./scan_and_analyze.sh <address> reuse-r
...
four characters (`1Be6` … `zfha`) against the reply.

## License

MIT — see [LICENSE](LICENSE). © 2026 Solitech.
